...
- [ ] Tune `preserving-stall-v1` rules toward the objective instead of pure win-rate cost: examine why the healthy-bench switch and the 0.15 finish rule sacrifice the stall mirror's own win; consider a target-quality guard on switching (e.g., only switch to a bench mon with a favorable matchup), and re-measure on frozen-ou-v2.
