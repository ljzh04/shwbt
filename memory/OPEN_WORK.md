...
- [x] Produce a real non-baseline candidate policy (`packages/agent/src/preserving-policy.ts`, registered `preserving-stall-v1`) and run it through the frozen-campaign gate. Measured via EXP-20260919-001: byte-identical outcomes vs control on frozen-ou-v1 and frozen-ou-v2 — KEEP, zero delta. Candidate retained in the registry as a control arm, not promoted. Note: zero delta was an artifact of the reducer discarding request `condition` HP (fixed in EXP-20260919-002 work).
- [x] Restore agent-visible HP from `|request|` roster `condition` fields (reducer `applyRequestRoster`/`applyRequestRosterFromJson` + regression test). Gate now discriminates: EXP-20260919-002 on frozen-ou-v2, candidate removes the catastrophic sweep (0.25->0.0) but is win-rate-negative (0.25 vs 0.5) — KEEP.
- [x] Rule-iteration attempt to recover the lost win: `preserving-stall-v2` (tempo-faint-lead 2) and `preserving-stall-v3` (finishOwnHpThreshold 0.2) both byte-identical to v1 on frozen-ou-v2 (EXP-20260919-003). Both knobs are inert on these seeds; the win-to-tie cost is a single preservation switch at T12 on the balance side (s2), i.e. preservation applied to an offensive team. Hand-tuning has hit diminishing returns on 4 seeds.
- [ ] Build a discriminating campaign where a low-HP switch / finish decision can actually change the outcome: scenarios with strong forced-switch pressure, knockout differentials, or policy-side opponents with varied switch seeds so deterministic-vs-deterministic lockstep cannot mask candidate behavior changes. (frozen-ou-v2 already discriminates candidate-vs-control now that HP is visible; add more seeds and forced-switch/knockout scenarios for statistical power.)
- [ ] Identity-gate preservation overrides to stall-shaped teams (roster recovery-profile heuristic), since the candidate demonstrably hurts the balance side's win on s2 (EXP-20260919-003); re-measure on frozen-ou-v2 seeds.
- [ ] Evaluate the candidate on the stall objective-vector control metrics (PP depletion, forced switches, own-faint preservation) in addition to winRate, since candidate v1 improves own faints 6->4 / 5->4 at a win-rate cost; decide what the announcement metric should be.
